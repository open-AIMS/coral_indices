CI_25_MA_baseline_models <- function() {
    ...
    CI_tryCatch({
        ...
        load(paste0(DATA_PATH, 'primary/disturbances.RData'))
            ...
            #Code was corrected in the analyses for the Macroalgae paper, and copied over from there 01/04/2026
            #Prepare the data ----
            disturbance <- disturbance.reef %>% filter(RANK=='1') %>%
            dplyr::select(P_CODE,REEF,DEPTH,VISIT_NO,DISTURBANCE)

            # baseline data for inshore reefs (shallow and deep)
            baseline.points.a <- points.analysis.data.transect %>%
            ungroup() |> 
            left_join(disturbance) %>%
            filter(!DISTURBANCE %in% c('s','f')) |> ## exclude post flood and storm obs as these are often outliers
            filter(!is.na(DEPTH)) |> #remove JCU sites with no depth
            ...
            #       REPORT_YEAR <= 2015,
            #       DEPTH < 9.1) |> 
            ## to include the first 3 years for reefs that only have 1 year in th baseline:
            group_by(REEF.d)|> 
            mutate(is_baseline = REPORT_YEAR %in% sort(unique(REPORT_YEAR))[1:3]) |> 
            ungroup() |> 
            filter(DEPTH < 9.1) |> 
            filter((REPORT_YEAR > 2005 & REPORT_YEAR <= 2015) | is_baseline == TRUE) |>
            ...
            # baseline data for offshore reefs
            baseline.points.b <- points.analysis.data.transect %>%
            ungroup() %>%
            left_join(spatial_lookup |>
                        dplyr::select(REEF, DEPTH, DEPTH.f, REEF.d, BIOREGION, BIOREGION.agg, NRM,Shelf)  |> 
                        distinct()) |> 
